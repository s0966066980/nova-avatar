...
的 Ollama 程式與 BGE-M3 模型檔，但另開 CPU
程序；Nova 原有的 Ollama 與 llama.cpp 語言模型服務不變。首次安裝與每位數位人的
檢索設定均預設關閉。
...
密碼。`up` 啟動 `nova-ragflow` Compose 專案與獨立的使用者層級 CPU 嵌入服務；
...
引擎和嵌入程序設有 CPU／記憶體上限，文件批量解析降至一次一份。即使如此，
...
2. 在 RAGFlow 模型供應者設定中新增 Ollama 執行個體：服務位址
   `http://host.docker.internal:11435`，嵌入模型 `bge-m3`。此 CPU 嵌入程序
...
- 每輪最多取 4 段、每段最多 800 字；檢索總逾時 8 秒（依本機 CPU BGE-M3
  實測由原先 2.5 秒調整）。檢索資料只放進
  本輪模型上下文，不寫進對話歷史。控制台的「檢索參考」只是片段預覽，
  不是正式引文，也不另行送入語音或看板。
- 沒有命中時仍可一般回答，但語音與控制台都標明「知識庫沒有找到相關資料」。
  服務停用、逾時或故障時，繼續原本回答流程，控制台顯示失敗狀態。
